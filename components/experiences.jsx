...
export default function Experiences (props){
  ...
  return (
    <section className=' py-12' id='experiences'>
        ...
        className={`section-title text-center lg:text-4xl md:text-3xl xs:text-2xl text-primaryColor font-bold mt-12 md:mb-20 xs:mb-10 uppercase ${props.isArabic ? '' : 'tracking-[1rem] xs:tracking-[0.5rem]'} ` }>Professional experiences</motion.h1>
            
        <div key={5487} className=' flex flex-row flex-wrap justify-center align-center gap-8 lg:px-20 md:px-16 sm:px-2'>
            
        {props.experiencesList && props.experiencesList.map((experience) => {
            console.log(experience.date_fin_experience)
                return <motion.div
                key={experience._id}
                initial = {{
                    x : 200,
