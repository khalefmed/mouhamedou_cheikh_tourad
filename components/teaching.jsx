...
export default function Teaching (props){
  ...
  return (
    <section className=' py-12' id='talks'>
        ...
        className={`section-title text-center lg:text-4xl md:text-3xl xs:text-2xl text-primaryColor font-bold mt-12 md:mb-20 xs:mb-10 uppercase ${props.isArabic ? '' : 'tracking-[1rem] xs:tracking-[0.5rem]'} ` }>Teaching</motion.h1>
            
        <div key={5487} className=' flex flex-row flex-wrap justify-center align-center gap-8 lg:px-20 md:px-16 sm:px-2'>
            
        {props.teachingList && props.teachingList.map((teaching) => {
                return <motion.div
                key={teaching._id}
                initial = {{
                    x : 200,
                ...
                whileInView = {{
                 ...
                 className=' actualite shadow-2xl flex flex-col justiyf-center align-center gap-2 py-2 px-2 rounded-lg w-[21rem] '>
                    ...
                    <div className='px-3 py-3 flex flex-col gap-2'>
                        <h2 className='text-[grey] text-dm text-start'>{teaching.year}</h2>
                        <h2 className='text-black font-semibold text-lg  text-start'>{teaching.module}</h2>
                        <h2 className='text-[grey] text-dm text-start'>{teaching.formation}</h2>
