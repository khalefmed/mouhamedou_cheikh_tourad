...
export default function ActualiteListe(props){

    const builder = imageUrlBuilder(props.sanityClient);
    ...
    console.log(props.sanityClient)


    console.log(props.actualites[1].image_actualite.asset._ref)
    console.log(props.actualites[1])
        ...
        <div className=' lg:px-20 md:px-16 sm:px-3 flex flex-row justify-center align-center flex-wrap gap-5 my-16'>
            
            {
                 props.actualites.map((actualite) => {
                    return <Link href={`actualites/${actualite._id}`}>
                    <motion.div
                    initial = {{
                        x : 0,
                        y : 0,
                    ...
                    className=' actualite shadow-2xl flex flex-col justiyf-center align-center gap-2 py-2 px-2 rounded-lg w-[21rem] '>
                        <img 
                            src={urlFor(actualite.image_actualite.asset).url()} 
                            alt="" 
                            className=' rounded-lg w-full max-h-[12rem] box-content' 
                        />
                        <div className='px-3 py-3 flex flex-col gap-2'>
                            <p className=' text-[grey] text-dm text-start'>{actualite['date_actualite']}</p>
                            <h2 className='text-black font-semibold text-lg  text-start'>{Substring(actualite[`titre_actualite_${props.i18n.language}`])}</h2>
                            <a href="produits/produits" className='text-green-900 font-semibold text-start w-full-5 my-2 text-lg'>{props.seeMore} </a>
                        </div>
                </motion.div>
                    </Link>
                })
